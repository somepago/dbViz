...
save_path_2 = args.model_path_2
save_path_2 = save_path_2.replace("/", "_").split(".")[0]

for run in range(args.epochs):

    pred_1 = torch.load(f"./predictions/{save_path}_{run}_preds.pth")
    pred_2 = torch.load(f"./predictions/{save_path_2}_{run}_preds.pth")
    iou += calculate_iou_simple(pred_1, pred_2)

print(save_path)
print(save_path_2)
print(iou/args.epochs)
