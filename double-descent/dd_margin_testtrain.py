...
if args.noise_rate > 0 and args.plot_method == 'train':
    ...
    if args.plot_method == 'train':
        unique_classes = {
            'aa' : avg_margin(l_all,l_all,trainloader,trainloader,args.num_iters, args.num_samples,net,device)
        } 
        if args.active_log:
                    wandb.log({
                        'aa': np.mean(unique_classes['aa']),
    ...
    elif args.plot_method == 'test_train':
        unique_classes = {
            'aa' : avg_margin(l_all_test,l_all,testloader,trainloader,args.num_iters, args.num_samples,net,device)
        } 
        if args.active_log:
                    wandb.log({
                        'aa': np.mean(unique_classes['aa']),
        ...
        unique_classes = {
            'aa' : avg_margin(l_all_test,l_all_test,testloader,testloader,args.num_iters, args.num_samples,net,device)
        } 
        if args.active_log:
                    wandb.log({
                        'aa': np.mean(unique_classes['aa']),
