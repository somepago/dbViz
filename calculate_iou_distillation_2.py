...
def get_paths(students, teachers):
    ...
    s_p = "./saved_models/og_distillation"
    print(s_p)
    for teacher in teachers:
        students_paths[teacher] = {}
        for student in students:
            students_paths[teacher][student] = []
            for seed in [1, 2, 3]:
                path = f"{s_p}/student={student}_teacher={teacher}_og_distillation_{seed}_cifar10.pth"
                students_paths[teacher][student].append(path)
                print(path)
...
print("Students")
for teacher in student_paths.keys():
    student_bounds[teacher] = {}
    for student in student_paths[teacher].keys():
        models = student_paths[teacher][student]
        all_seeds = []
        for model_seed in models:
            print(model_seed)
            args.net = student
        ...
        student_bounds[teacher][student] = torch.stack(all_seeds).reshape(-1)


iou_mat = torch.zeros(len(teachers), len(students))
...
for teacher in student_paths.keys():
    ...
    s_i = 0
    for student in student_paths[teacher].keys():
        student_db = student_bounds[teacher][student]
        iou_mat[t_i][s_i] = calculate_iou_simple(teacher_db, student_db)
        print(teacher, student, iou_mat[t_i][s_i])
        s_i += 1
